...
uint8 notificationType = 0; // 0: broadcast, 1: unicast
/*********************************************************************
 * TYPEDEFS
 */
...
static bStatus_t DSS_sendNotification(uint8 *pValue, uint16 len)
{
  ...
  uint16 offset = 0;
  uint8 i = 0;
  static uint8 connectionIndex = 0;
  ...
  // Check for broadcast or unicast
  if (pValue[0] == 0x2D && pValue[1] == 0x2D) // Broadcast condition
  {
    notificationType = 0; // Set to broadcast
  }
  else if (pValue[0] == 0x2B && pValue[1] == 0x2B) // Unicast condition
  {
    notificationType = 1; // Set to unicast
    connectionIndex = pValue[4]; // Get the connection index from the 5th byte
  }
  ...
  if ( pAttr != NULL )
  {
      ...
      if (notificationType == 0)
      {
          ...
          {
            gattCharCfg_t *pItem = &( dss_dataOut_config[i] );
            // If the connection has registered for notifications
            if ( ( pItem->connHandle != LINKDB_CONNHANDLE_INVALID ) &&
                 ( pItem->value == GATT_CLIENT_CFG_NOTIFY) )
            {
              // Find out what the maximum MTU size is for each connection
      ...
      // Handle unicast notifications
      else if (notificationType == 1)
      {
         gattCharCfg_t *pItem = &( dss_dataOut_config[connectionIndex] );
        // Ensure the connection index is valid
        if (connectionIndex < MAX_NUM_BLE_CONNS &&
            pItem->connHandle != LINKDB_CONNHANDLE_INVALID &&
            pItem->value == GATT_CLIENT_CFG_NOTIFY)
        {
          // Only process for the specified connection
          if (1)
          {
            status = linkDB_GetInfo(pItem->connHandle, &connInfo);
            offset = 0;
